...
$pageSql = "SELECT * FROM ".$tableName." LEFT JOIN truangan ON truangan.id_ruangan=$tableName.id_ruangan LEFT JOIN tkondisi ON tkondisi.id_kondisi=$tableName.id_kondisi WHERE jenis_aset='Non-Aset'";
...
if(isset($_POST['qcari'])){
  ...
  where jenis_aset='Non-Aset' AND ($field1 like '%$qcari%' or $field2 like '%$qcari%' or $field3 like '%$qcari%' or nama_ruangan like '%$qcari%' or kondisi like '%$qcari%') ";
}
...
<table class="table table-bordered table-striped">
  <tr>
    <td><h1><b>Data <?php echo $formName;?></b> <form class="navbar-search pull-right"  method="POST" action="?page=<?php echo $formName?>-Non-Aset">
	...
	<td>
	 <div class="pagination">
			  ...
			  <li><a href="?page=<?php  $g = $hal-1;
				if ($g<=0)
				{$g=0;}	
			  echo $formName;?>-Non-Aset&hal=<?php echo $g?>">Prev</a></li>
				<?php
				for ($h = 1; $h <= $max; $h++) {
					$list[$h] = $row * $h - $row;
					echo " <li><a href='?page=".$formName."-Non-Aset&hal=$list[$h]'>$h</a></li> ";
				}
				?>	 
              </ul>
            </div>
